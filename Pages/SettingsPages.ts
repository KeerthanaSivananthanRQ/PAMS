...
async CreateIncidentType(){
    ...
    await this.page.getByPlaceholder('Enter # to add new keyword').fill('#TestKeyword');
    await this.page
...
async EditIncidentType(IncidentTypeName: string) {
    const editedIncidentTypeName = IncidentTypeName + IncidentTypeName;
     await this.page
  .locator('div._card_1wp7q_74', { hasText: IncidentTypeName })
  .locator('span._editIcon_1wp7q_121')
  .click();
    await expect(this.page.getByRole('heading', { name: 'Edit Incident' })).toBeVisible();
    const nameInput = this.page.getByRole('textbox', { name: 'incidentTypeTitle' });
    await nameInput.click();
    await nameInput.press('Control+A')
    await nameInput.press('Backspace');
    await nameInput.fill(editedIncidentTypeName);
    await this.page.getByRole('button', { name: 'Save Changes' }).click();
}
...
async DeleteIncidentType(IncidentTypeName:string){
    const editedIncidentTypeName = IncidentTypeName + IncidentTypeName;
    await this.page
    .locator('div', { hasText: editedIncidentTypeName })
...
async AssertStatusAdded(StatusName:string){
    ...
    await this.page.locator('div.ant-select[name="status"]').click();
...
async deletestatus(StatusName:string){
    ...
    await this.page.getByRole('button', { name: 'Statuses' }).click();
    await this.page
    .locator('div', { hasText: StatusName })
...
async editRank(RankName:string){
    ...
    .first()
    .click();
    await expect(this.page.getByRole('heading', { name: 'Edit Rank' })).toBeVisible();
    const nameInput = this.page.getByRole('textbox', { name: 'rankTitle' });
    await nameInput.click();
    await nameInput.press('Control+A')
    await nameInput.press('Backspace');
    await nameInput.fill(editedRankName);
    await this.page.getByRole('button', { name: 'Save Changes' }).click(); 
}
...
async EditChecklist(ChecklistName:string){
    const editedChecklistName = ChecklistName + ChecklistName;
     await this.page
    .locator('div', { hasText: ChecklistName })
    .locator('span', { hasText: 'edit_square' })
    .first()
    .click();
     await expect(this.page.getByRole('heading', { name: 'Edit Checklist' })).toBeVisible();
     const nameInput = this.page.getByRole('textbox', { name: 'checklistTitle' });
    await nameInput.click();
    await nameInput.press('Control+A')
    await nameInput.press('Backspace');
    await nameInput.fill(editedChecklistName);
    await this.page.getByRole('button', { name: 'Save Changes' }).click(); 
}
async AssertEditedChecklist(ChecklistName:string){
    const editedRankName = ChecklistName + ChecklistName;
    await this.page.getByRole('row', { name: ChecklistName }).isVisible();   
}
async DeleteChecklist(ChecklistName:string){
    const editedRankName = ChecklistName + ChecklistName;
    await this.page
    .locator('div', { hasText: ChecklistName })
    .locator('span', { hasText: 'delete' })
...
async ValidateDuplicateIncidentType(IncidentTypeName: string){
    await this.CreateIncidentType();
    await this.settingsIcon.click();
    await this.page.getByRole('button', { name: 'Incident Types' }).click();
    await this.page.locator('button span:text("Incident Type")').click();
    await this.page.waitForTimeout(3000);
...
async AssertValidateDuplicateIncidentType() {
